...
render_config() {
  docker run \
    ...
    "$NGINX_IMAGE" \
    /bin/sh -c "envsubst '\$SITE_DOMAIN \$PANEL_HTTPS_PORT \$SUBS_HTTPS_PORT \$PANEL_ALLOWED_CIDR \$PANEL_PATH_PREFIX \$SUBS_PATH_PREFIX \$PANEL_BACKEND_SCHEME \$PANEL_BACKEND_HOST \$PANEL_BACKEND_PORT \$SUBS_BACKEND_SCHEME \$SUBS_BACKEND_HOST \$SUBS_BACKEND_PORT' < /etc/nginx/templates/nginx.conf.template" \
...
assert_static_tls_config() {
  ...
  if ! grep -q "proxy_ssl_name panel-backend.test;" "$TMP_DIR/rendered-nginx.conf"; then
    fail "panel upstream TLS identity must use PANEL_BACKEND_HOST"
  fi
  if ! grep -q "proxy_ssl_name subs-backend.test;" "$TMP_DIR/rendered-nginx.conf"; then
    fail "subscription upstream TLS identity must use SUBS_BACKEND_HOST"
  fi
}
...
start_nginx() {
  ...
  docker run \
    ...
    "$NGINX_IMAGE" \
    /bin/sh -c "envsubst '\$SITE_DOMAIN \$PANEL_HTTPS_PORT \$SUBS_HTTPS_PORT \$PANEL_ALLOWED_CIDR \$PANEL_PATH_PREFIX \$SUBS_PATH_PREFIX \$PANEL_BACKEND_SCHEME \$PANEL_BACKEND_HOST \$PANEL_BACKEND_PORT \$SUBS_BACKEND_SCHEME \$SUBS_BACKEND_HOST \$SUBS_BACKEND_PORT' < /etc/nginx/templates/nginx.conf.template > /etc/nginx/nginx.conf && nginx -t && nginx -g 'daemon off;'" \
    >/dev/null || {
