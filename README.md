...
Пустые пути `/SUBS_PATH_PREFIX` и `/SUBS_PATH_PREFIX/` не проксируются в backend и сразу отдают кастомную `404`-страницу. Некорректные `subid`, на которые backend отвечает `400` или `404`, тоже подхватываются `error_page` и показывают кастомную страницу вместо сырого `Error!`.
